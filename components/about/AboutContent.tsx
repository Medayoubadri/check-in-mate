...
// import MiniDashboard from "./about-sections/MiniDashboard";
...
const defaultAbout: About[] = [
  ...
  //   id: "dashboard",
  //   color: "purple",
  //   title: "A Dashboard So Clean, Even Teachers Approve",
  //   description:
  //     "The dashboard presents attendance data in a way that actually makes sense. You get metrics, reports, and a sleek UI that doesn't make your eyes bleed. Is it perfect? Almost. But at least it's not a spreadsheet from 2003.",
  //   isActive: true,
  // },
];
...
const getFeatureContent = (featureId: string) => {
  switch (featureId) {
    ...
    //           neonClassName="!w-[40rem]"
    //           className="-top-40 absolute"
    //           neonColor={defaultAbout[3].color}
    //         />
    //         <motion.div className="group bg-background dark:bg-neutral-950 dark:opacity-80 rounded-lg w-[550px] 1xl:w-[700px] h-[500px] 1xl:h-[600px] transition-transform translate-x-4 hover:translate-x-0 duration-500 cursor-default">
    //           <MiniDashboard />
    //           <div
    //             className={`top-0 md:top-0 left-1/2 -z-10 absolute bg-${defaultAbout[3].color}-500 opacity-10 group-hover:opacity-[0.15] blur-[60px] md:blur-[90px] rounded-[100%] w-full h-[200px] md:h-[400px] transition-all -translate-x-1/2 -translate-y-1/2 duration-700 pointer-events-none`}
    //           />
    //         </motion.div>
    //       </div>
    //     </TooltipProvider>
